	...
	public function __construct($url, array $options = []) {
		...
		if(isset($data['scheme']) && strtolower($data['scheme']) == 'ftps') {
			if(isset($data['port']) && $data['port']) {
				$this->conn = ftp_ssl_connect($data['host'], $data['port'], $options['timeout']);
				if(!$this->conn) {
			...
			if(isset($data['port']) && $data['port']) {
				$this->conn = ftp_connect($data['host'], $data['port'], $options['timeout']);
				if(!$this->conn) {
					throw new \Exception("Could not connect to 'ftp://{$data['host']}:{$data['port']}'");
				}
			}
			else {
				$this->conn = ftp_connect($data['host'], 21, $options['timeout']);
				if(!$this->conn) {
